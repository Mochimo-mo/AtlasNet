...
class Att_Agg(nn.Module):
    def __init__(self):
        super(Att_Agg, self).__init__()

        fc = torch.nn.Conv1d(1024, 1024, 1)



class MVR_AtlasNet(nn.Module):
    def __init__(self, num_points = 2048, bottleneck_size = 1024, nb_primitives = 5, pretrained_encoder = False, cuda=True):
        super(MVR_AtlasNet, self).__init__()
    ...
    def forward(self,x1,x2,x3):
        ...
        obj_features0 = torch.cat((x1[:,None,:], x2[:,None,:], x3[:,None,:]),1).contiguous()

        scores = F.softmax(obj_features0, dim=1)

        obj_features2=obj_features0*scores
        obj_features3=torch.sum(obj_features2,dim=1)
        x=obj_features3
