        ...
        if self.SVR:
            if self.train:
                ...
                if N < 10:
                    ...
                    im1 = Image.open(os.path.join(fn[0], "0" + str(N+1) + ".png"))
                    im2 = Image.open(os.path.join(fn[0], "0" + str(N + 2) + ".png"))
                else:
                    im = Image.open(os.path.join(fn[0],  str(N) + ".png"))
                    im1 = Image.open(os.path.join(fn[0], str(N+ 1) + ".png"))
                    im2 = Image.open(os.path.join(fn[0], str(N + 2) + ".png"))

                im = self.dataAugmentation(im) #random crop
                im1 = self.dataAugmentation(im1)
                im2 = self.dataAugmentation(im2)
            else:
                if self.idx < 10:
                    im = Image.open(os.path.join(fn[0], "0" + str(self.idx) + ".png"))
                    im1 = Image.open(os.path.join(fn[0], "0" + str(self.idx+1) + ".png"))
                    im2 = Image.open(os.path.join(fn[0], "0" + str(self.idx +2) + ".png"))
                else:
                    im = Image.open(os.path.join(fn[0],  str(self.idx) + ".png"))
                    im1 = Image.open(os.path.join(fn[0], str(self.idx + 1) + ".png"))
                    im2 = Image.open(os.path.join(fn[0], str(self.idx + 2) + ".png"))
                im = self.validating(im) #center crop
                im1 = self.validating(im1)  # center crop
                im2 = self.validating(im2)  # center crop
            data1 = self.transforms(im) #scale
            data2= self.transforms(im1)
            data3 = self.transforms(im2)
