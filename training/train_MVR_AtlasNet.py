...
parser.add_argument('--model_preTrained_AE', type=str, default='../trained_models/ae_atlasnet_25.pth',
                    help='model path')
...
parser.add_argument('--fix_decoder', type=bool, default=True,
                    help='if set to True, on the the resnet encoder is trained')
...
for i, data in enumerate(dataloader_test, 0):
    ...
    dist1, dist2, dist3, dist4 = distChamfer(points.transpose(2, 1).contiguous(), pointsReconstructed)
...
# Create network
network = MVR_AtlasNet(num_points=opt.num_points, nb_primitives=opt.nb_primitives)
...
for epoch in range(opt.nepoch):
    ...
    for i,data in enumerate(dataloader, 0):
        ...
        pointsReconstructed = network(img,img1,img2)
        dist1, dist2,dist3,dist4 = distChamfer(points, pointsReconstructed)
        loss_net = (torch.mean(dist1)) + (torch.mean(dist2))
        ...
        # VIZUALIZE
        if i % 5 <= 0:
            vis.image(img[0].data.cpu().contiguous(), win='0', opts=dict(title="INPUT IMAGE TRAIN0"))
            vis.image(img1[0].data.cpu().contiguous(), win='1', opts=dict(title="INPUT IMAGE TRAIN1"))
            vis.image(img2[0].data.cpu().contiguous(), win='2', opts=dict(title="INPUT IMAGE TRAIN2"))
        ...
        epoch, i, len_dataset / 7, loss_net.item(), trainloss_accs / trainloss_acc0))
